...
- Never report real secrets or keys, just flag the pattern and location.
...
Generate the full report in the format defined in `references/report-format.md`.

**Save location:** Write the report to `docs/codebase/SECURITY.md` within the scanned
repository (create the `docs/codebase/` directory if it does not exist). If a previous
`SECURITY.md` exists at that path, overwrite it — the report always reflects the latest scan.
